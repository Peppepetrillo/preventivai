...
export default function ArchivioPreventivi() {
  ...
  const preventiviFiltrati = preventivi.filter((preventivo) =>
    preventivo.cliente?.toLowerCase().includes(ricerca.toLowerCase())
  );

  function coloreStato(stato) {
    switch (stato) {
      case "Bozza": return "bg-yellow-500";
      case "Inviato": return "bg-blue-500";
      case "Accettato": return "bg-green-500";
      case "Completato": return "bg-slate-500";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 to-slate-900 text-white p-5 pb-32">
      ...
      <div className="mb-8">
        <h1 className="text-4xl font-black mb-3">Archivio Preventivi</h1>
        <p className="text-slate-400">Tutti i preventivi salvati</p>
      </div>
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-3 mb-3">
                  <h2 className="text-2xl font-bold">{preventivo.cliente}</h2>
                ...
                <div className="mt-2 space-y-1">
                  <p className="text-slate-500 text-sm">{preventivo.numero || "PREV-000"}</p>
                  <p className="text-slate-400">{preventivo.data}</p>
                </div>
                <p className="text-green-400 text-2xl font-bold mt-4">€ {preventivo.totale}</p>
              </div>
